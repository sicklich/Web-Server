...
2. 参考设计模式的 **依赖倒转** 原则，在 `EventLoop` 模块(高层)下添加了一个 `Dispatcher` 模块(抽象类)，并以 `Dispatcher` 模块为父类，实现 `EpollDispatcher、PollDispatcher、SelectDispatcher` 三个子模块(低层)，以实现对高层代码的直接复用；
3. 采用主从反应堆模型，将与客户端的连接和业务处理分割，以提升并发性能；
4. 采用 `One loop per thread` 模式，并利用线程池进行统一管理，在提升并行效率的前提下减少了线程创建与销毁的开销；
5. 利用回调函数的优势，减少代码复用，增加灵活性及可读性；
6. 利用强类型枚举区分不同事件，可以通过位运算一次传递多个不同事件的同时提高代码的安全性；
7. 采用单例模式设计日志类，以提供可执行异步操作的日志对象，通过饿汉模式构建日志类对象，保证线程安全，并提供日志自动备份功能，以防日志文件过大。
...
5. 非静态成员引用必须与特定对象相对<br>
**问题描述**: 在使用类静态成员函数时，尝试通过非静态类成员变量（类类型）调用其非静态函数时报错。<br>
**问题分析**: 由于非静态成员属于某个对象，而类静态成员属于该类，因此静态成员函数只能访问静态成员变量。<br>
**解决方案一**: 将调用的非静态函数也改为静态函数，`Log::addTask("400 Bad Request", 1, conn->m_log)`。<br>
**解决方案二**: 声明一个实例对象来调用 该非静态函数，`conn->m_log->addTask("400 Bad Request", 1, parameter)`。<br><br>
